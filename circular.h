...
public:
    ...
    : array_(alloc_.allocate(capacity,0)), array_size_(capacity),
    head_(1), tail_(0), contents_size_(0) {
        array_size_1_=array_size_-1;
    }
    ...
    inline reference operator[](size_type n) {
        return array_[(n+head_)&array_size_-1];
        //return at_unchecked(n);
    }
//    inline reference operator[](size_type n) {
//        return array_[n&array_size_1_];
//        //return at_unchecked(n);
//    }
...
public:
    ...
    // Typedefs
    typedef CircularBuffer<T, Alloc>    self_type;
    ...
    void push_back(const value_type &item) {
        array_[contents_size_&(array_size_-1)] = item;
        contents_size_=contents_size_+1;        

    }
    void move_to(size_type pos) {
        contents_size_=pos;

    }

    inline reference operator[](size_type n) {
        return array_[(n)&(array_size_-1)];
        //return at_unchecked(n);
    }
...
private:
    ...
    void destroy_all_elements() {
        for (size_type n = 0; n < array_size_; ++n) {
            alloc_.destroy(array_ + n);
        }
    }
