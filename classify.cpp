...
int MATCHcls::Classify(int *newBeat,int rr, int noiseLevel, int *beatMatch, int *fidAdj,
	int init)
	{
	...
	if(init)
		...
		runCount = 0 ;
        brIndex = 0;
        lastIsoLevel=0, lastRhythmClass = UNKNOWN, lastBeatWasNew = 0 ;
		DomMonitor(0, 0, 0, 0, 1) ;
		return(0) ;
		}

	hfNoise = HFNoiseCheck(newBeat) ;	// Check for muscle noise.  //local
	rhythmClass = rythmcnk.RhythmChk(rr) ;			// Check the rhythm.    //rythmchk.c

	// Estimate beat features.

	AnalyzeBeat(newBeat, &onset, &offset, &isoLevel,                //analbeat.c
	...
	BestMorphMatch(newBeat,&morphType,&matchIndex,&mi2,&shiftAdj) ;      //match.c
	...
	if((matchIndex < MATCH_LIMIT) && (rhythmClass == PVC) &&
		...
		{
		morphType = NewBeatType(newBeat) ;                                 //match.c
		lastBeatWasNew = 1 ;
		}
	...
	// Match if within standard match limits.
		...
		UpdateBeatType(morphType,newBeat,mi2,shiftAdj) ;                   //match.c
	...
	DomType = domType = DomMonitor(morphType, rhythmClass, beatWidth, rr, 0) ;   //local
	domWidth = GetBeatWidth(domType) ;                                           //match.c

	// Compare the beat type, or actual beat to the dominant beat.

	if((morphType != domType) && (morphType != MAXTYPES))
		domIndex = DomCompare(morphType,domType) ;                               //match.c
	else if(morphType == MAXTYPES)
		domIndex = DomCompare2(newBeat,domType) ;                                //match.c
	else domIndex = matchIndex ;

	// Update post classificaton of the previous beat.

	PostClassify(RecentTypes, domType, RecentRRs, beatWidth, domIndex, rhythmClass) ;    //postclas.c

	// Classify regardless of how the morphology
	// was previously classified.

	tempClass = TempClass(rhythmClass, morphType, beatWidth, domWidth,                   //local
	...
	if((beatClass == UNKNOWN) && (morphType < MAXTYPES))
		...
		if((runCount >= 3) && (domType != -1) && (beatWidth < domWidth+BEAT_MS20))
			SetBeatClass(morphType,NORMAL) ;                                               //match.c
		...
		else if(rythmcnk.IsBigeminy() == 1)
			{
			if((rhythmClass == PVC) && (beatWidth > BEAT_MS100))
				SetBeatClass(morphType,PVC) ;                                              //match.c
	...
	*beatMatch = morphType ;
	...
	if(CheckPostClass(morphType) == PVC)                                                    //postclas.c
   {
       // printf("PVC:%d\n",PVC);
       return(PVC) ;
    }
...
int MATCHcls::TempClass(int rhythmClass, int morphType,
...
*/
int MATCHcls::DomMonitor(int morphType, int rhythmClass, int beatWidth, int rr, int reset)
	{
...
int MATCHcls::GetNewDominantType(void)
	{
	return(NewDom) ;
	}

int MATCHcls::GetDomRhythm(void)
	{
...
void MATCHcls::AdjustDomData(int oldType, int newType)
	{
	int i ;
...
void MATCHcls::CombineDomData(int oldType, int newType)
	{
	int i ;
...
int MATCHcls::GetRunCount()
	{
	int i ;
	for(i = 1; (i < MAXTYPES) && (RecentTypes[0] == RecentTypes[i]); ++i) ;
	return(i) ;
	}
